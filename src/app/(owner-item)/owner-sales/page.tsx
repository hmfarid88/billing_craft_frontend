...
const Page = () => {
    ...
    useEffect(() => {
        fetch(`${apiBaseUrl}/sales/groupSaleSummaryCurrentMonth?username=${username}`)
            .then(res => res.json())
            .then(data => {
                setAllProducts(data);
            ...
            <div className="flex flex-col p-5">
                <div className="flex justify-between items-center pl-5 pr-5 pt-5">
                    <label className="input input-bordered flex max-w-xs items-center gap-2">
                     ...
                     <DateToDate routePath="/datewise-owner-sales" />
                    <div className="flex items-center gap-3">
                        <button onClick={handlePrint} className='btn btn-ghost btn-square'><FcPrint size={36} /></button>
                    </div>
                </div>

                <div ref={contentToPrint} className="flex flex-col p-2 mt-10 items-center justify-center">
                    <h4 className="font-bold">SALES & PROFIT</h4>
                    <h4 className="text-lg"><CurrentMonthYear /></h4>
